...
export const ModalOverlay = (props: ModalOverlayProps) => {
    return (
        <AriaModalOverlay
            ...
            className={(state) =>
                cx(
                    "fixed inset-0 flex min-h-dvh w-full items-center justify-end bg-black/50 backdrop-blur-sm pl-6 outline-hidden ease-linear md:pl-10 z-[100]",
...
export const Modal = (props: ModalProps) => (
    <AriaModal
        ...
        className={(state) =>
            cx(
                "inset-y-0 right-0 h-full w-full max-w-lg shadow-2xl transition",
...
export const Dialog = (props: DialogProps) => (
    <AriaDialog
        ...
        {...props}
        className={cx(
            "relative flex size-full flex-col items-start gap-6 overflow-y-auto bg-background ring-1 ring-border outline-hidden",
            props.className,
        )}
    />
);
...
const Content = ({ role = "main", ...props }: ComponentPropsWithRef<"div">) => {
    return <div role={role} {...props} className={cx("flex size-full flex-col gap-6 overflow-y-auto overscroll-auto px-4 md:px-6 pb-6", props.className)} />;
};
...
const Footer = (props: ComponentPropsWithRef<"footer">) => {
    return <footer {...props} className={cx("w-full p-4 shadow-[inset_0px_1px_0px_0px] shadow-border md:px-6", props.className)} />;
};
